...
def beam_section_properties(
    ...
    for i in range(n_nodes):
        ...
        x_centroid = (
            2.0 * np.sum(x_mid * t_sk * ds) + x[0] * A_fs + x[-1] * A_rs
        ) / A_tot
...
def solve_bending_beam(
    y_nodes: np.ndarray, EI: np.ndarray, F: np.ndarray
) -> np.ndarray:
...
def solve_torsion_beam(
    y_nodes: np.ndarray, GJ: np.ndarray, T: np.ndarray
) -> np.ndarray:
...
def compute_combined_stress(
    ...
    use_torsion = (
        phi_full is not None and GJ is not None and A_e is not None and t_sk is not None
    )
    ...
    for i in range(n_nodes - 1):
        ...
        sigma_b = abs(M_x) * (h[i] / 2.0) / max(I_xx[i], 1e-30) + abs(M_z) * (
            w[i] / 2.0
        ) / max(I_zz[i], 1e-30)
