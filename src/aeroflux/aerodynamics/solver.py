...
import numpy as np
from aeroflux.aerodynamics.aic import assemble_aic, build_rhs
from aeroflux.aerodynamics.force import (
    near_field_forces,
    profile_drag_cd0,
    trefftz_cdi,
)
from aeroflux.aerodynamics.panel_mesh import build_panel_mesh
from aeroflux.atmosphere.isa import isa
from aeroflux.geometry.wing import WingGeometry
from aeroflux.registry import DisciplineBase, register
from aeroflux.schemas.types import AERO_OUTPUT_SPEC, AeroOutput
from aeroflux.schemas.validation import (
...
class VLMDiscipline(DisciplineBase):
    ...
    def __call__(self, **inputs: Any) -> AeroOutput:
        ...
        if M >= 1.0:
            raise ValueError(
                f"Mach number {M:.2f} >= 1.0. Prandtl-Glauert correction is valid only for subsonic flow."
            )
        ...
        CL, CDi_kj, CM, lift_N, drag_i_N, span_cl, span_load, span_drag = (
            near_field_forces(
                mesh,
                gamma,
                rho,
        ...
        CL = (
            2.0
            * float(np.trapezoid(span_cl[wing.n_panels :] * wing.chord_cp, wing.y_cp))
            / mesh.S_ref
        )
        ...
        tc_stations = np.array([
            float(sec.thickness_at(np.array([0.35]))[0])
            for sec in wing.airfoil_sections
        ])
