...
from aeroflux.geometry.wing import WingGeometry, build_wing_geometry
from aeroflux.registry import DisciplineBase, register
from aeroflux.schemas.types import AeroOutput, StructuralOutput
...
class AerostructDiscipline(DisciplineBase):
    ...
    def __call__(self, **inputs: Any) -> AerostructOutput:
        ...
        for _n_iter in range(1, _MAX_ITER + 1):
            ...
            new_delta_twist = np.interp(
                wing.y_stations, y_nodes, np.asarray(struct.span_twist, dtype=float)
            )
