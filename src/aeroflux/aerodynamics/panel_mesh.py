...
import numpy as np
from aeroflux.geometry.airfoil import interpolate_camber
...
def build_panel_mesh(wing: WingGeometry) -> PanelMesh:
    ...
    z_at_nodes = np.array([
        chord_nodes[j]
        * float(interpolate_camber(y_nodes[j], wing.y_stations, sections_list, x_qc)[0])
        for j in range(n + 1)
    ])  # shape (n+1,)
    ...
    z_cp = np.array([
        wing.chord_cp[j]
        * float(
            interpolate_camber(wing.y_cp[j], wing.y_stations, sections_list, x_tqc)[0]
        )
        for j in range(n)
    ])  # shape (n,)
