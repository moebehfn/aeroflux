...
import numba
import numpy as np
from aeroflux.aerodynamics.aic import _biot_savart_finite, _biot_savart_semi_infinite

if TYPE_CHECKING:
    from aeroflux.aerodynamics.panel_mesh import PanelMesh
...
def near_field_forces(
    ...
    if isinstance(alpha_rad, np.ndarray):
        ...
        V_inf = np.stack(
            [V_ms * np.cos(alpha_rad), np.zeros(n), V_ms * np.sin(alpha_rad)], axis=1
        )
        # lift_dir = [-sin(alpha), 0, cos(alpha)] for each panel
        lift_dir = np.stack(
            [-np.sin(alpha_rad), np.zeros(n), np.cos(alpha_rad)], axis=1
        )
...
def trefftz_cdi(
    ...
    CDi_tp = -np.sum(
        gamma * (w_tp * (mesh.yB - mesh.yA) - v_tp * (mesh.zB - mesh.zA))
    ) / (V_ms**2 * S_ref)
    return float(CDi_tp)
