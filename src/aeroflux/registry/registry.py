...
"""Fidelity plugin registry for aeroflux disciplines.
...
def get_discipline(family: str, key: str) -> type:
    ...
    if key not in DISCIPLINE_REGISTRY[family]:
        ...
        raise KeyError(
            f"Unknown key {key!r} for family {family!r}. Available keys: {available}."
        )
...
class DisciplineBase(ABC):
    """Abstract base class for all aeroflux discipline implementations.
