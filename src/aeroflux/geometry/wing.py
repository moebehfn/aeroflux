...
import numpy as np
from aeroflux.geometry.airfoil import blended_profile, interpolate_camber
...
class WingGeometry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.deflection_z is None:
            ...
            if deflection_z.shape != (self.n_panels + 1,):
                raise ValueError(
                    f"deflection_z must have shape ({self.n_panels + 1},), got {deflection_z.shape}"
                )
    ...
    def _section_at_y(self, y: float, n_profile_points: int) -> dict:
        ...
        x_norm, z_norm = blended_profile(
            y, self.y_stations, list(self.airfoil_sections), n_profile_points
        )
    ...
    def plot_wing_3d(
        ...
        def mirror(sections):
            return [
                {"y": -s["y"], "x": s["x"], "z": s["z"]} for s in reversed(sections)
            ]
        ...
        y_nodes = _cosine_y_nodes(self.half_span, self.n_panels)
        node_sections = [
            self._section_at_y(float(y), n_profile_points) for y in y_nodes
        ]
        ...
        for j, sec in enumerate(all_nodes):
            if alpha_deg != 0.0:
                ...
                sec["x"], sec["y"], sec["z"] = r.apply(
                    np.matrix([sec["x"], [sec["y"]] * len(sec["x"]), sec["z"]]).T
                ).T
...
def build_wing_geometry(
    ...
    if not np.isclose(y_stations[-1], half_span):
        raise ValueError(
            f"y_stations[-1] must equal half_span={half_span}, got {y_stations[-1]}"
        )
    ...
    for arr_name, arr in [
        ...
        if arr.shape != (n_sections,):
            raise ValueError(
                f"{arr_name} must have shape ({n_sections},), got {arr.shape}"
            )
    if np.any(chord <= 0.0):
        raise ValueError("All chord values must be positive")
    if len(airfoil_sections) != n_sections:
        raise ValueError(
            f"airfoil_sections must have {n_sections} entries, got {len(airfoil_sections)}"
        )
